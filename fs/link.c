...
PUBLIC int do_unlink(){
	...
	if(strcmp(pathname,"/")==0){
		printl("FS: do_unlink() unable to delete root file\n");
		return -1;
	}

	int inode_nr=search_file(pathname);
	if(inode_nr==INVALID_INODE){
		printl("FS:do_unlink() unable to find the file inode with path: %s\n",pathname);
		return -1;
	}
	...
	if(pin->i_cnt>1){
		printl("cannot remove %s because this file is using by other task\n",filename);
		return -1;
	}
	...
	*/
	...
	int s=2+sb->nr_imap_sects+byte_idx/SECTOR_SIZE;//2: bootsec+sb_sect
	RD_SECT(pin->i_dev,s);
